...
def train_xgboost(cleaned_csv: Path, target: str, test_size: float = 0.2, seed: int = 42) -> None:
    ...
    # Deadline-friendly XGBoost config
    model = XGBRegressor(
        n_estimators=600,
        max_depth=4,
        learning_rate=0.05,
    ...
    metrics.update({"model": "XGBoost", "target": target, "test_size": test_size, "seed": seed})

    # Save model + results
    Path("models").mkdir(parents=True, exist_ok=True)
    joblib.dump(pipe, Path(f"models/xgb_{safe_name(target)}.joblib"))

    save_metrics(metrics, Path(f"results/metrics/xgb_metrics_{safe_name(target)}.json"))

    plot_pred_vs_actual(
        y_test, preds,
        Path(f"results/plots/pred_vs_actual_xgb_{safe_name(target)}.png"),
        title=f"Pred vs Actual (XGB) - {target}"
    )

    plot_feature_importance(
        pipe.named_steps["model"].feature_importances_,
        Path(f"results/plots/feat_importance_xgb_{safe_name(target)}.png"),
        title=f"Feature Importance (XGB) - {target}",
        top_k=20
    )
